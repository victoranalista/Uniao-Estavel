...
export class PDFGenerator implements GenerateDeclarationUseCase {
  ...
  async execute(declaration: Declaration & { averbation?: string }, seal: string): Promise<Uint8Array> {
    ...
    const registrarFunction = OFICIAIS_REGISTRADORES[declaration.registrarName] || 'Oficial Registrador(a)';
    ...
    return new Promise((resolve, reject) => {
      ...
      const backgroundImagePath = path.join(process.cwd(), 'public', 'fundo.png');
      ...
      // Add averbation if exists
      if (declaration.averbation) {
        doc.moveDown(2)
          .fontSize(14)
          .font('Helvetica-Bold')
          .text('AVERBAÇÃO:', { align: 'left' })
          .moveDown()
          .fontSize(12)
          .font('Helvetica')
          .text(declaration.averbation, {
            align: 'justify',
            lineGap: 2
          });
      }
