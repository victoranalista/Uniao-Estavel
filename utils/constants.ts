...
export const REGISTRY_TYPES = ['Nascimento', 'Casamento', 'Óbito'] as const;
...
const fetchStatesFromAPI = async (): Promise<string[]> => {
  ...
  return states
    .flat()
    .map((state: IBGEState) => state.sigla)
    .sort();
};
...
const fetchCitiesFromAPI = async (uf: string): Promise<string[]> => {
  const { data: page } = await brasilAPI.IBGE.listFederativeUnitMinicipalities(
    uf,
    undefined,
    { take: 1000 }
  );
...
export const getNationalitiesByGender = (
  gender: 'M' | 'F'
): readonly string[] =>
