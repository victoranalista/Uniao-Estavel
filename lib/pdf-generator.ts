...
const loadBackgroundImage = async (pdfDoc: PDFDocument): Promise<PDFImage | null> => {
  try {
...
const createPageWithBackground = (pdfDoc: PDFDocument, backgroundImage: PDFImage | null): PDFPage => {
...
const drawHeader = (page: PDFPage, book: string, term: string, helveticaBold: any, showTitle: boolean = true): number => {
...
const drawTextLine = (page: PDFPage, line: Array<{text: string, bold: boolean}>, x: number, y: number, helveticaFont: any, helveticaBold: any): void => {
  let currentX = x;
...
const drawJustifiedText = (page: PDFPage, line: Array<{text: string, bold: boolean}>, margin: number, y: number, maxWidth: number, helveticaFont: any, helveticaBold: any): void => {
...
const drawSignatures = (page: PDFPage, declarationData: z.infer<typeof declarationSchema>, registrarFunction: string, helveticaFont: any, helveticaBold: any, startY: number): void => {
  const { width: pageWidth } = page.getSize();
...
const getFinalSeal = (data: unknown) => {
  const typedData = data as any;
  return typedData.stamp || getDefaultSeal();
};

const getRegistrarFunction = (registrarName: string) => {
  return OFICIAIS_REGISTRADORES[registrarName] || 'Oficial Auxiliar';
};
...
const buildCompletePdf = async (
  ...
  validatedData: z.infer<typeof declarationSchema>, 
  config: { pageWidth: number; pageHeight: number; margin: number; lineHeight: number; signatureAreaHeight: number; marginBottom: number },
  book: string, 
  term: number, 
  finalSeal: string, 
...
const buildDeclarationText = (validatedData: z.infer<typeof declarationSchema>, term: number, finalSeal: string) => {
  return generateDeclarationText(validatedData)
...
const drawTextContent = (
  ...
  config: { pageWidth: number; margin: number; lineHeight: number; marginBottom: number; signatureAreaHeight: number }, 
  fonts: { helveticaFont: any; helveticaBold: any }, 
  pdfDoc: PDFDocument, 
  backgroundImage: PDFImage | null, 
  book: string, 
...
const drawSingleLine = (
  ...
  line: Array<{text: string, bold: boolean}>, 
  currentY: number, 
  config: { margin: number; pageWidth: number; lineHeight: number }, 
  fonts: { helveticaFont: any; helveticaBold: any }, 
  isLastLine: boolean
) => {
...
const calculateLineWidth = (line: Array<{text: string, bold: boolean}>, fonts: { helveticaFont: any; helveticaBold: any }) => {
  return line.reduce((width: number, part: { text: string; bold: boolean }) => {
