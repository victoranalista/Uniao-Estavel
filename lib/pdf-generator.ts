...
const formatDateExtended = (date: string) => {
  ...
  const yearWords =
    year === 2025 ? 'dois mil e vinte e cinco' : year.toString();
...
const getPropertyRegimeText = (regime: string) => {
  const regimes: Record<string, string> = {
    COMUNHAO_PARCIAL:
      '**COMUNHÃO PARCIAL DE BENS**, nos termos do artigo 1.725 do Código Civil, de modo que todos os bens adquiridos onerosamente durante a constância da união pertencerão a ambos em partes iguais, cabendo sua administração indistintamente a ambos os companheiros',
    SEPARACAO_TOTAL:
      '**SEPARAÇÃO TOTAL DE BENS**, de forma que os bens não se comunicam, sendo mantidos os patrimônios individuais, separados e independentes',
    PARTICIPACAO_FINAL:
      '**PARTICIPAÇÃO FINAL NOS AQUESTOS**, de forma que cada parte conserva seu próprio patrimônio, sendo que, após a dissolução da união, cada um tem direito à metade dos bens adquiridos onerosamente durante a convivência',
    COMUNHAO_UNIVERSAL:
      '**COMUNHÃO UNIVERSAL DE BENS**, de forma que todos bens, adquiridos antes ou durante a união, onerosos ou gratuitos, se comunicam integralmente'
  };
...
const wrapFormattedText = (
  formattedParts: Array<{ text: string; bold: boolean }>,
  maxWidth: number,
  fontSize: number,
  normalFont: any,
  boldFont: any
...
const buildDeclarationText = (
  declaration: DeclarationData,
  isUpdate = false
) => {
  const currentDate = getCurrentDate();
  const registrarFunction =
    (REGISTRY_OFFICERS as Record<string, string>)[declaration.registrarName] ||
    'Oficial Auxiliar';
...
const createPageWithBackground = (
  pdfDoc: PDFDocument,
...
const drawTextLine = (
  page: PDFPage,
  line: Array<{ text: string; bold: boolean }>,
  x: number,
  y: number,
  helveticaFont: any,
  helveticaBold: any
...
const drawSignatures = (
  ...
  const secondPersonWidth = helveticaBold.widthOfTextAtSize(
    secondPersonName,
    11
  );
  page.drawText(secondPersonName, {
  ...
  const registrarNameWidth = helveticaBold.widthOfTextAtSize(
    registrarNameText,
    11
  );
  page.drawText(registrarNameText, {
  ...
  const registrarFunctionWidth = helveticaFont.widthOfTextAtSize(
    registrarFunction,
    10
  );
  page.drawText(registrarFunction, {
...
const drawOnlyRegistrarSignature = (
  page: PDFPage,
...
const calculateLineWidth = (
  line: Array<{ text: string; bold: boolean }>,
  fonts: { helveticaFont: any; helveticaBold: any }
) => {
...
const drawEndLineDashes = (
  page: PDFPage,
  line: Array<{ text: string; bold: boolean }>,
  currentY: number,
  config: { margin: number; pageWidth: number },
  fonts: { helveticaFont: any; helveticaBold: any }
) => {
  ...
  if (dashesNeeded > 0) {
    ...
    page.drawText(dashes, {
      x: config.margin + lineWidth,
      y: currentY,
      size: 11,
...
const drawSingleLine = (
  ...
  if (line.length > 1 && !isLastLine) {
    drawJustifiedText(
      ...
      config.margin,
      currentY,
      config.pageWidth - config.margin * 2,
      fonts.helveticaFont,
      fonts.helveticaBold
    );
  } else {
...
const shouldCreateNewPage = (
  ...
  return (
    ...
    config.marginBottom + config.signatureAreaHeight
  );
};

const drawTextContent = (
  page: PDFPage,
  textLines: Array<Array<{ text: string; bold: boolean }>>,
  config: {
    pageWidth: number;
    margin: number;
    lineHeight: number;
...
const buildCompletePdf = async (
  pdfDoc: PDFDocument,
  fonts: { helveticaFont: any; helveticaBold: any },
  backgroundImage: PDFImage | null,
  validatedData: DeclarationData,
  config: {
    pageWidth: number;
    pageHeight: number;
    margin: number;
  ...
  const useExistingNumbers =
    isUpdate && validatedData.termNumber && validatedData.bookNumber;
  ...
  const textLines = wrapFormattedText(
    formattedParts,
    maxWidth,
    11,
    fonts.helveticaFont,
  ...
  const { finalPage, finalY } = drawTextContent(
    currentPage,
    textLines,
    config,
    fonts,
  ...
  if (isUpdate) {
    drawOnlyRegistrarSignature(
      finalPage,
      validatedData.registrarName,
      registrarFunction,
      fonts.helveticaFont,
      fonts.helveticaBold,
      finalY
...
const validatePdfData = (data: unknown) => {
  ...
  if (!result.success)
    throw new Error(`Dados inválidos: ${result.error.message}`);
  return result.data;
};
...
const getRegistrarFunction = (registrarName: string): string => {
  return (
    (REGISTRY_OFFICERS as Record<string, string>)[registrarName] ||
    'Oficial Auxiliar'
  );
};
...
export const generateSecondCopyPdfAction = async (
  declarationData: unknown,
  existingBook: string,
  existingTerm: string
) => {
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Erro ao gerar segunda via do PDF'
    };
  }
};
